...
const InventoryMovementForm = () => {
  ...
  useEffect(() => {
    if (isOpenNotification) {
      ...
      }, 3000); // ตรงกับ duration ของ AutoDismissAlert

      return () => clearTimeout(timer);
    }
  }, [isOpenNotification]);

  // ย้าย useCallback ขึ้นมาด้วย
  ...
  // ย้าย useEffect สำหรับ click outside ขึ้นมาด้วย
  useEffect(() => {
  ...
  // Handle form item change with unit price calculation
  const handleFormItemChange = (id: number, field: string, value: string | number) => {
    const updatedItems = formItems.map(item => {
      if (item.id === id) {
        ...
        // Recalculate prices when quantity or purchase price changes
        if (field === 'quantity' || field === 'cost') {
          const newQuantity = field === 'quantity' ? newValue : item.quantity;
          const newPurchasePrice = field === 'cost' ? newValue : item.cost;

          // Calculate unit price (purchase price divided by quantity)
  ...
  // Calculate total purchase price for all items
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. ตรวจสอบการเลือกวัตถุดิบและจำนวน
    ...
    // 2. ถ้าเป็นการนำเข้า ต้องตรวจสอบราคาและแหล่งที่มา
    if (formType === 'IMPORT') {
      // ตรวจสอบแหล่งที่มา
      if (!source.trim()) {
        setTypeNotification('error');
...
return (
  <div className="bg-white shadow-lg rounded-lg p-6 max-w-4xl mx-auto">
        ...
        <h2 className="text-2xl font-semibold mb-6 text-gray-800">บันทึกการนำเข้า/ออก</h2>
